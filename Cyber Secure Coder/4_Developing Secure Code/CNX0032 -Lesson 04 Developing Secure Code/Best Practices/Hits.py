...
'''
from threading import Thread            # Using Python Standard Threading Module
import time
...
def ThreadRace(threads):
    ''' Create Thread that will cause a race condition'''
...
def main():
    ...
    for i in range(100):
        t = Thread(target=ThreadRace, args=(i,))
        threads.append(t)
        t.start()
